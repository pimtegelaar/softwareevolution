...
public Figure mapD2T(m: \field(Type \type, list[Expression] fragments)) = t("field","purple",[mapD2T(\type)]+[mapD2T(f)|f<-fragments] + [mapD2T(modifier) | modifier <- m@modifiers?[]]);
public Figure mapD2T(\initializer(Statement initializerBody)) = t("initializer","purple",[mapD2T(initializerBody)]);
public Figure mapD2T(m: \method(Type \return, str name, list[Declaration] parameters, list[Expression] exceptions, Statement impl)) = t("method: "+name,"purple",[mapD2T(modifier) | modifier <- m@modifiers?[]] + [mapD2T(impl)] + [ mapD2T(param) | param <- parameters]);
...
public Figure mapD2T(m: \variables(Type \type, list[Expression] \fragments)) = t("variables","purple",[mapD2T(modifier) | modifier <- m@modifiers?[]] + [mapD2T(\type)]+[mapD2T(fragment) | fragment <- fragments]);
